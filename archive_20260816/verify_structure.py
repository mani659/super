...
# fix-verifydup: was ["cab_watcher.py", "cab_watcher.py"] — a duplicate
# entry checking the same filename twice, functionally harmless but
# confusing (looked like it was meant to check a filename variant that
# doesn't exist). Left as a list in case a real variant needs adding later.
cab_candidates = ["cab_watcher.py"]
found_cab = None
...
check("cab_super\\cab_watcher file", found_cab is not None,
      found_cab if found_cab else "not found — expected cab_watcher.py")
...
try:
    from ghost_super import ghost_sniper
    check("ghost_super.ghost_sniper imports OK", True)
except Exception as e:
    check("ghost_super.ghost_sniper imports OK", False, str(e)[:80])

try:
    from ghost_super import sniper_watcher
    check("ghost_super.sniper_watcher imports OK", True)
except Exception as e:
    check("ghost_super.sniper_watcher imports OK", False, str(e)[:80])

try:
    from ghost_super import ghost_cache
    check("ghost_super.ghost_cache imports OK", True)
except Exception as e:
    check("ghost_super.ghost_cache imports OK", False, str(e)[:80])

try:
    import importlib.util
    cab_file = found_cab or "cab_watcher.py"
