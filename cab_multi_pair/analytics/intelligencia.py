...
def _calculate_adx(df: pd.DataFrame, period: int = 14) -> float:
    ...
    try:
        ...
        # Wilder's Smoothing approx via rolling sum
...
def get_entry_intelligence(symbol: str) -> dict:
    ...
    try:
        # 1. Active Portfolio Exposure
        positions = mt5.positions_get()
        if positions:
            snapshot["Active_Risk_Trades"] = len([p for p in positions if p.magic == config.MAGIC_NUMBER])

        # 2. Volatility State (Normalized ATR)
        ...
        if rates_h1 is not None and len(rates_h1) >= 30:
            ...
            current_atr = df_h1['tr'].iloc[-1]
            mean_atr = df_h1['tr'].mean()
            
            if current_atr > (mean_atr * 1.5): snapshot["H1_ATR_State"] = "EXPANSION_HIGH"
            elif current_atr < (mean_atr * 0.7): snapshot["H1_ATR_State"] = "COMPRESSION_LOW"
        ...
        # Using EURUSDm as an inverse proxy for DXY trend
        eu_rates = mt5.copy_rates_from_pos("EURUSDm", mt5.TIMEFRAME_H4, 1, 3)
        if eu_rates is not None and len(eu_rates) >= 3:
            if eu_rates[-1]['close'] < eu_rates[0]['open']: snapshot["Macro_DXY_Proxy"] = "USD_BULLISH"
            else: snapshot["Macro_DXY_Proxy"] = "USD_BEARISH"

        # Using USTECm (Nasdaq) as a proxy for Global Risk-On / Risk-Off
        nq_rates = mt5.copy_rates_from_pos("USTECm", mt5.TIMEFRAME_H4, 1, 3)
        if nq_rates is not None and len(nq_rates) >= 3:
            if nq_rates[-1]['close'] > nq_rates[0]['open']: snapshot["Macro_Risk_Sentiment"] = "RISK_ON"
            else: snapshot["Macro_Risk_Sentiment"] = "RISK_OFF"
